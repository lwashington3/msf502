...
\begin{itemize}
	...
	\begin{itemize}
		\item $\bar{p}$ is the point estimator of the population proportion $p$.
	\end{itemize}
...
\begin{itemize}
	...
	\begin{equation}
		...
		\label{eq:population-proportion-desired-margin-of-error}
	\end{equation}
	(where $\bar{p}$ is the sample proportion),
	we may rearrange to get
	\begin{equation}
		n = \left( \frac{z_{\alpha/2}}{D} \right)^{2} \bar{p}(1 - \bar{p})
		\label{eq:sample-proportion-size}
	\end{equation}
	...
	\begin{equation}
		...
		\label{eq:population-proportion-confidence-interval-sample-size}
	\end{equation}
